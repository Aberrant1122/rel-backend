...
// If using ethereal email for testing without env vars, generate account dynamically
...
const getTransporter = async () => {
    if (testAccountPromise) {
        return await testAccountPromise;
    }
    return transporter;
};
...
const sendInvoiceEmail = async (reservation) => {
    try {
        ...
        const info = await mailTransporter.sendMail({
            from: process.env.SMTP_FROM || '"Reservations" <no-reply@example.com>',
